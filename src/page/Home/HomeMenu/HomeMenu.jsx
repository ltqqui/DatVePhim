...
export default function HomeMenu(props) {
  ...
  const renderHomeMenu = () => {
    return heThongRapChieu?.map((heThongRap, index) => {
      return <Fragment  key={index}>
        <Tabs.TabPane  style={{width:'100%', padding:'0'}} tab={
           <div className='flex justify-center'>
           <img src={heThongRap.logo} className='rounded-full' width={50} height={50} alt='' />
           <div style={{ display: 'flex', alignItems: 'center' }}>
             <p style={{ lineHeight: '10px', padding: '0px', marginLeft: 0, marginBottom: 0, marginLeft: '10px', fontWeight: '500' }}>{heThongRap.tenHeThongRap}</p>
           </div>
         </div>
        } width='50' key={index}>
          <div  >
                  ...
                  <Tabs.TabPane style={{height:700, overflowY:'scroll'}}  tab={ 
                    <div style={{ display: 'flex' ,width:'300px' }}>
                      ...
                      {cumRap?.danhSachPhim.map((phim,index)=>{
                        return <Fragment key={index}>
                            <div style={{width:'100%',margin:0,padding:'0px', display:'flex', marginTop:15}}>
                                <img src={phim.hinhAnh} style={{width:90, height:100}} alt="" />
                                <div className='ml-3'>
                                  <p className='font-bold my-0 text-lg'>{phim.tenPhim}</p>
  ...
  return (
    <div className='mx-72' style={{ height: '700px', }}>
